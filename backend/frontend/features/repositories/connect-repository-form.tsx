...
export function ConnectRepositoryForm() {
  ...
  return (
    <form onSubmit={submit} noValidate className="mt-6 space-y-5">
      <div><label htmlFor="repo-url" className="mb-2 block type-compact-strong text-text-secondary">HTTPS GitHub repository URL</label><div className="relative"><Github className="absolute left-2.5 top-[13px] size-3.5 text-muted-foreground max-[820px]:top-[15px]" /><Input id="repo-url" value={url} onChange={(event) => { setUrl(event.target.value); setValidation(null); }} placeholder="https://github.com/owner/repository" className="pl-8 type-mono" aria-invalid={Boolean(error)} aria-describedby={error ? "repo-url-help repo-url-recovery" : "repo-url-help"} autoFocus /></div><p id="repo-url-help" role={error ? "alert" : undefined} className={`mt-2 type-compact ${error ? "text-danger" : "text-muted-foreground"}`}>{error ?? "Use a complete github.com owner/repository URL. A trailing .git suffix is accepted."}</p>{error ? <p id="repo-url-recovery" className="mt-2 type-compact text-muted-foreground">Correct the URL or resolve the reported backend access issue, then submit again.</p> : null}</div>
      <p role="status" aria-live="polite" className="sr-only">{connect.isPending ? "Repository connection submitted. Waiting for the backend response." : ""}</p>
