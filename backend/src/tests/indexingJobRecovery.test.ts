...
test("an active lease is not recovered and heartbeat extends its expiry", async () => {
  ...
  fixture.advance(50);
  assert.equal(await fixture.store.heartbeatJob(created.jobId, "active-worker", 100), true);
...
test("expired lease rejects renewal and becomes recoverable", async () => {
  const fixture = recoveryFixture();
  const created = await fixture.create();
  await fixture.store.claimNextJob("abandoned-worker", 100);
  fixture.advance(100);

  assert.equal(await fixture.store.heartbeatJob(created.jobId, "abandoned-worker", 100), false);
...
test("an abandoned worker cannot mutate a job after a replacement claims it", async () => {
  const fixture = recoveryFixture();
  const created = await fixture.create();
  await fixture.store.claimNextJob("worker-a", 100);
  await fixture.store.markRunning(created.jobId, "scan", "worker-a");
  fixture.advance(101);
  await fixture.recover();
  fixture.advance(25);
  await fixture.store.claimNextJob("worker-b", 100);
  await fixture.store.markRunning(created.jobId, "clone", "worker-b");

  assert.equal(await fixture.store.updateProgress(created.jobId, 25, "scan", "worker-a"), null);
  assert.equal(await fixture.store.markSucceeded(created.jobId, "worker-a"), null);
  assert.equal(await fixture.store.markFailed(created.jobId, {
    code: "stale_worker",
...
test("non-retryable failures remain terminal", async () => {
  ...
  assert.equal(await fixture.store.scheduleRetry(created.jobId, "worker-a", failure, 0), null);
